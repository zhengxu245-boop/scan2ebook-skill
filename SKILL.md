---
name: scan2ebook
description: "把扫描版/图片版中文书籍 PDF（无文字层）端到端转成结构化简体 Markdown、EPUB 与按章节分好的 PDF：视觉大模型 OCR → opencc 繁转简 → LLM 校对分章 → pandoc 出书 → reportlab 出带目录书签的 PDF。当用户要 OCR 一本书、繁体转简体、把旧扫描书整理成电子书时使用，含断点续跑、确定性杂质清洗与模型选型踩坑清单。"
whenToUse: "当用户要把扫描版中文书籍 PDF 转成可检索的简体电子书（Markdown/EPUB/PDF/DOCX），或要求 OCR 一本书、繁体转简体、按章分好的 PDF、把旧书整理成结构化电子书时使用。"
metadata:
  pipeline: [ocr, simplify, detect, proofread, deliver, clean, pdf]
...
（`reportlab` + `pypdf` 只在出 PDF 时需要；`Pillow` 只在 OCR 截断切半兜底时需要。）
...
7. **PDF 交付（按章节分好）**：`python scripts/pdf.py`（加 `--config book-xxx.toml` 指定另一本；`--level 1` 只按「部」切）→
   - `output/pdf/<title>.pdf`：单文件 = 封面 + 目录（**带页码**）+ 全文 + 「部/章」**PDF 书签**；
   - `output/pdf/<title>/split/NN_<标题>.pdf`：每章一个独立 PDF，便于分章入库。
   实现为纯 Python（reportlab + pypdf），中文用系统字体（宋体/冬青黑）；**同样先过杂质闸**，脏 md 拒绝出 PDF。`ebook-ocr` 项目已用此脚本产出《过犹不及》163 页/24 章、《在祈祷中相遇》76 页/27 章。
...
**坑 #7（2026-09-10）：出 PDF 别走 HTML→Chrome headless 打印路线。** 本机（macOS + Chrome 152，且已有常驻 Chrome 实例）实测 `--headless=new` / `--headless` 加各种禁特性开关**全部挂起**（30s 无输出，进程不退；注意 macOS 自带 bash 没有 `timeout` 命令，脚本里必须给 `subprocess.run(..., timeout=)` 设超时，否则整条流水线卡死）。**对策：用 `scripts/pdf.py`（reportlab 直出 PDF）**——纯 Python、秒级完成，自带头页目录页码与「部/章」书签，中文用系统字体（`/System/Library/Fonts/Supplemental/Songti.ttc`，reportlab 以 `subfontIndex=0` 注册 TTC 即可）。若非得用浏览器渲染，先确认 `wkhtmltopdf`/LaTeX/typst 是否可用，别默认 Chrome 行。
